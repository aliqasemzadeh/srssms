...
new class extends Component
{
    ...
    #[On('panels.administrator.finance-management.payment.index.refresh')]
...
<div>
    @php
        ...
        $gatewayMethods = PaymentGateways::gatewayMethodOptions();
    @endphp

    <x-slot name="title">{{ __('general.payments') }} - {{ config('app.name') }}</x-slot>
            ...
            <div class="sm:hidden shrink-0">
                <flux:dropdown align="end">
                    ...
                    <flux:menu>
                        <flux:menu.item
                            icon="settings"
                            href="{{ route('panels.administrator.finance-management.payment.setting.index') }}"
                            wire:navigate
                        >
                ...
                <flux:dropdown align="end" class="shrink-0 ms-auto">
                    ...
                    <flux:popover class="w-[min(100vw-2rem,20rem)] max-h-[70vh] overflow-y-auto flex flex-col gap-4">
                        ...
                        <flux:select wire:model.live="method" variant="listbox" searchable placeholder="{{ __('general.method') }}..." clearable>
                            @foreach ($gatewayMethods as $methodKey => $methodLabel)
                        ...
                        <flux:select wire:model.live="currencyId" variant="combobox" :filter="false" clearable placeholder="{{ __('general.currency') }}...">
                            ...
                            @foreach ($this->currencies as $currency)
                                <flux:select.option value="{{ $currency->id }}" wire:key="payment-currency-{{ $currency->id }}">
                ...
                <flux:table.rows>
                    @forelse ($this->deposits as $deposit)
                        @php
                            ...
                            $canCheck = in_array($deposit->status, [DepositStatusEnum::Pending, DepositStatusEnum::Processing], true);
                        @endphp
                        ...
                        <flux:table.row>
                            <flux:table.cell colspan="12">
                                <div class="flex flex-col items-center justify-center gap-2 py-10 text-center">
                                    <flux:icon.credit-card variant="outline" class="size-8 text-zinc-400" />
    ...
    <livewire:finance-management.payment.check :key="'finance-payment-check'" />
</div>
