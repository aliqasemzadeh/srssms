...
new class extends Component
{
    ...
    #[On('panels.administrator.user-management.user.wallet.withdrawal.index.refresh')]
...
<div>
    ...
    <x-slot name="title">{{ __('general.withdrawals') }} - {{ $currencySymbol }} - {{ $user->full_name }} - {{ config('app.name') }}</x-slot>
    ...
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            @can('finance-management.withdrawal.create')
            <flux:button class="shrink-0" variant="primary" color="teal" icon="plus" wire:click="$dispatch('panels.administrator.user-management.user.wallet.withdrawal.create.assign-data')">
                        ...
                        <flux:table.row :key="$withdrawal->id">
                            ...
                            <flux:table.cell align="end">
                                <div class="flex justify-end gap-2">
                                    @can('finance-management.withdrawal.edit')
                                    <flux:tooltip content="{{ __('general.edit') }}">
                                        <flux:button size="xs" variant="primary" color="blue" icon="pencil" icon:variant="outline" wire:click="$dispatch('panels.administrator.user-management.user.wallet.withdrawal.edit.assign-data', { withdrawal: {{ $withdrawal->id }} })" />
                                    </flux:tooltip>
                                    @endcan
                                    @can('finance-management.withdrawal.delete')
                                    <flux:tooltip content="{{ __('general.delete') }}">
                                        <flux:button size="xs" variant="danger" icon="trash" icon:variant="outline" wire:click="$dispatch('panels.administrator.user-management.user.wallet.withdrawal.delete.assign-data', { withdrawal: {{ $withdrawal->id }} })" />
                                    </flux:tooltip>
                                    @endcan
                                </div>
                            </flux:table.cell>
    ...
    <livewire:user-management.user.wallet.withdrawal.create :user="$user" :wallet="$wallet" :key="'user-wallet-withdrawal-create-'.$wallet->id" />
    <livewire:user-management.user.wallet.withdrawal.edit :user="$user" :wallet="$wallet" :key="'user-wallet-withdrawal-edit-'.$wallet->id" />
    <livewire:user-management.user.wallet.withdrawal.delete :user="$user" :wallet="$wallet" :key="'user-wallet-withdrawal-delete-'.$wallet->id" />
</div>
