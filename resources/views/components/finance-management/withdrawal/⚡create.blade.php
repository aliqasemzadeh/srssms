...
new class extends Component
{
    ...
    public function assignData(): void
    {
        ...
        Flux::modal('finance-management.withdrawal.create')->show();
    }
...
<flux:modal name="finance-management.withdrawal.create" flyout position="right" class="space-y-6">
    <div>
    ...
    <form wire:submit="save" class="space-y-6">
        <flux:select wire:model.live="form.user_id" variant="combobox" :filter="false" label="{{ __('general.user') }}">
            ...
            @foreach ($this->users as $userOption)
                <flux:select.option value="{{ $userOption->id }}" wire:key="withdrawal-create-user-{{ $userOption->id }}">
                    {{ $userOption->label }}
                </flux:select.option>
            @endforeach
        </flux:select>
        ...
        <flux:select wire:model.live="form.wallet_id" variant="combobox" :filter="false" label="{{ __('general.wallet') }}" :disabled="blank($form->user_id)">
            ...
            @foreach ($this->wallets as $walletOption)
                <flux:select.option value="{{ $walletOption->id }}" wire:key="withdrawal-create-wallet-{{ $walletOption->id }}">
                    {{ $walletOption->label }}
                </flux:select.option>
            @endforeach
        </flux:select>
        ...
        <flux:select wire:model="form.user_account_id" variant="combobox" :filter="false" label="{{ __('general.user_account') }}" :disabled="blank($form->wallet_id)">
            ...
            @foreach ($this->userAccounts as $accountOption)
                <flux:select.option value="{{ $accountOption->id }}" wire:key="withdrawal-create-account-{{ $accountOption->id }}">
