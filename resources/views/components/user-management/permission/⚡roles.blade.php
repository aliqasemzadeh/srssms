...
new class extends Component
{
    ...
    public function confirm(string $action, ?int $id = null): void
    {
        ...
        Flux::modal('user-management.permission.roles.confirm')->show();
    }
    ...
    public function apply(): void
    {
        ...
        match ($this->pending_action) {
            'grant' => Role::findById($this->pending_id)->givePermissionTo($this->permission),
            'revoke' => Role::findById($this->pending_id)->revokePermissionTo($this->permission),
            'grant-all' => $this->grantable->each(fn (Role $role) => $role->givePermissionTo($this->permission)),
            'revoke-all' => $this->granted->each(fn (Role $role) => $role->revokePermissionTo($this->permission)),
        };
        ...
        Flux::modal('user-management.permission.roles.confirm')->close();
...
<div>
    ...
    <flux:modal name="user-management.permission.roles.confirm" class="min-w-[22rem] space-y-6">
        <div>
