...
new class extends Component
{
    ...
    public function save(): void
    {
        ...
        $currentIds = $this->permission->users()->pluck('id')->map(fn ($id) => (int) $id);

        User::query()
            ->whereIn('id', $selectedIds->diff($currentIds))
            ->get()
            ->each(fn (User $user) => $user->givePermissionTo($this->permission));

        User::query()
            ->whereIn('id', $currentIds->diff($selectedIds))
            ->get()
            ->each(fn (User $user) => $user->revokePermissionTo($this->permission));

        $this->dispatch('panels.administrator.user-management.permission.index.refresh');

        Flux::modals()->close();

        Flux::toast(__('general.permission_users_updated'));
    }
