...
new class extends Component
{
    ...
    public function confirm(string $action, ?int $id = null): void
    {
        ...
        Flux::modal('user-management.role.users.confirm')->show();
    }
    ...
    public function apply(): void
    {
        ...
        match ($this->pending_action) {
            'grant' => User::query()->find($this->pending_id)?->assignRole($this->role),
            'revoke' => User::query()->find($this->pending_id)?->removeRole($this->role),
            'grant-all' => $this->grantable->each(fn (User $user) => $user->assignRole($this->role)),
            'revoke-all' => $this->granted->each(fn (User $user) => $user->removeRole($this->role)),
        };
        ...
        Flux::modal('user-management.role.users.confirm')->close();
...
<div>
    ...
    <flux:modal name="user-management.role.users.confirm" class="min-w-[22rem] space-y-6">
        <div>
