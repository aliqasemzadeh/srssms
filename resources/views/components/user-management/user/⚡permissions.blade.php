...
new class extends Component
{
    ...
    public function confirm(string $action, ?int $id = null): void
    {
        ...
        Flux::modal('user-management.user.permissions.confirm')->show();
    }
    ...
    public function apply(): void
    {
        ...
        match ($this->pending_action) {
            'grant' => $this->user->givePermissionTo($this->pending_id),
            'revoke' => $this->user->revokePermissionTo(Permission::findById($this->pending_id)),
            'grant-all' => $this->grantable->each(fn (Permission $permission) => $this->user->givePermissionTo($permission)),
            'revoke-all' => $this->granted->each(fn (Permission $permission) => $this->user->revokePermissionTo($permission)),
        };

        $granting = $this->isGranting;

        $this->granted_ids = $this->user->permissions()->pluck('id')->map(fn ($id) => (int) $id)->all();
        ...
        Flux::modal('user-management.user.permissions.confirm')->close();
...
<div>
    ...
    <flux:modal name="user-management.user.permissions.confirm" class="min-w-[22rem] space-y-6">
        <div>
